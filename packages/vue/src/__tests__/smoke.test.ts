...
import { BarSeries, CandlestickSeries, ChartContainer, LineSeries, PieSeries, catppuccin } from '@wick-charts/vue';
...
describe('Vue wrapper smoke', () => {
  ...
  it('mounts CandlestickSeries with initial data and draws fillRects immediately', async () => {
    const App = defineComponent({
      ...
      setup() {
        return () =>
          h(ChartContainer, { theme: catppuccin.theme }, () => [h(CandlestickSeries, { data: candlestickData })]);
      },
    });
  ...
  it('mounts LineSeries with initial data and draws lineTo primitives', async () => {
    const App = defineComponent({
      ...
      setup() {
        return () => h(ChartContainer, { theme: catppuccin.theme }, () => [h(LineSeries, { data: lineData })]);
      },
    });
  ...
  it('mounts BarSeries (multi-layer) and draws bars per layer', async () => {
    ...
    const App = defineComponent({
      ...
      setup() {
        return () => h(ChartContainer, { theme: catppuccin.theme }, () => [h(BarSeries, { data: twoLayers })]);
      },
    });
    ...
    expect(main.__spy!.countOf('fill') + main.__spy!.countOf('fillRect')).toBeGreaterThanOrEqual(2);
    wrapper.unmount();
  });
  ...
  it('mounts PieSeries with initial data and draws arc primitives per slice', async () => {
    const App = defineComponent({
      ...
      setup() {
        return () => h(ChartContainer, { theme: catppuccin.theme }, () => [h(PieSeries, { data: pieData })]);
      },
    });
  ...
  it('reactive data updates produce additional draws', async () => {
    ...
    const App = defineComponent({
      components: { ChartContainer, LineSeries },
      setup() {
        return () => h(ChartContainer, { theme: catppuccin.theme }, () => [h(LineSeries, { data: data.value })]);
      },
    });
  ...
  it('unmount removes both canvases from the DOM', async () => {
    ...
    const App = defineComponent({
      components: { ChartContainer, CandlestickSeries },
      setup() {
        return () => h(ChartContainer, { theme: catppuccin.theme }, () => [h(CandlestickSeries, { data: data.value })]);
      },
    });
