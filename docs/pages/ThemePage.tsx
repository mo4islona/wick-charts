...
import type { JsonValue } from '../components/theme-editor/themeJson';
...
export function ThemePage({
  ...
  // Build the preview ChartTheme from the current JSON via createTheme. If
  // validation fails (e.g. missing `background` after a destructive edit),
  // fall back to the page theme so previews don't crash.
  const customTheme = useMemo(() => {
    try {
      const cfg = value as unknown as ThemeConfig;
      if (!cfg || typeof cfg !== 'object' || typeof cfg.background !== 'string') return theme;
